...
export default function Dessins() {
  ...
  // Récupérer les dessins depuis l'API
  useEffect(() => {
    fetchDrawings();
  }, []);
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          {loading ? (
            <div className="text-center py-20">
              <div className="text-foreground text-xl">Chargement des dessins...</div>
            </div>
          ) : drawings.length === 0 ? (
            <div className="text-center py-20">
              <p className="text-foreground text-xl mb-4">Aucun dessin disponible</p>
              <p className="text-gray-600 dark:text-gray-400">Les dessins ajoutés par l'admin apparaîtront ici</p>
            </div>
          ) : (
              ...
              {drawings.map((drawing, index) => (
                ...
                >
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-20 p-6 text-center">
                    <h3 className="text-off-white font-bold text-xl mb-2">{drawing.title}</h3>
                    {drawing.description && (
                      <p className="text-off-white/80 text-sm mb-4">{drawing.description}</p>
                    )}
        ...
        >
          <div className="relative max-w-3xl w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="bg-white dark:bg-gray-900 rounded-2xl overflow-hidden border-2 border-navy/40 dark:border-off-white/40 shadow-2xl">
              ...
              <div className="p-8">
                <h2 className="text-2xl font-bold text-foreground mb-4">{selectedDrawing.title}</h2>
                {selectedDrawing.description && (
                  <p className="text-gray-600 dark:text-gray-400 mb-4">{selectedDrawing.description}</p>
                )}
              </div>
            </div>
          </div>
      ...
      <section className="py-20 bg-gradient-to-br from-navy to-black">
        <div className="max-w-4xl mx-auto px-6 lg:px-8 text-center">
          ...
          <p className="text-xl text-off-white/80 mb-8">
            Contactez-nous pour transformer ces dessins en tatouages uniques
          </p>
          <Link
            href="/#contact"
